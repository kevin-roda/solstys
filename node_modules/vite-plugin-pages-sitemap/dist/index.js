...
const stream_1$2 = require$$0__default["default"];
...
function requireSitemapSimple () {
	...
	(function (exports) {
		...
		const path_1 = require$$0__default$2["default"];
		const stream_1 = require$$0__default["default"];
		const util_1 = require$$5__default["default"];
		const url_1 = require$$3__default["default"];
...
function getResolvedPath(filename, extension, dest) {
  ...
  return node_path.resolve(`${getDestPath(dest)}/${filenameWithExtension}`);
}
function writeXmlFile(resolvedPath, str, options) {
  node_fs.writeFileSync(resolvedPath, options.readable ? format(str) : str);
}
...
`).concat(`Sitemap: ${getFinalSitemapPath(options)}`);
  node_fs.writeFileSync(resolvedPath, str);
}
...
function generateSitemap(options) {
  ...
  if (!resolvedOptions.routes.length)
    return;
  if (!node_fs.existsSync(getDestPath(resolvedOptions.dest)))
    node_fs.mkdirSync(getDestPath(resolvedOptions.dest));
